const config = {
    //endpoint: 's3.amazonaws.com',   // Put you region
    accessKeyId: 'AKIAVDJJZI3IIIPO4PEX',       // Put you accessKeyId
    secretAccessKey: '0H+Lv3M8P2GRh8Jiv00UrknC+Q6whe0OQUrOj9Os',   // Put you accessKeyId
    Bucket: 'content-modernization-chandan',         // Put your bucket name
    signatureVersion: 'v4',
    region: 'us-east-1'           // Put you region
}
const dynamoDBName = "content_modernization";
...
async function uploadFile() {
    ...
    s3.upload({
        ...
        if (err) {
            ...
            addPost(data.Location.replace(fileName, ''), fileName);
            checkForModeration(fileName);
        }
...
function upload2() {
    s3.getSignedUrlPromise('putObject', {
        Bucket: 'content-modernization-chandan',
        Key: fileName,
        Expires: 60 * 5
    }).then((url) => {
        var fileInput = document.getElementById("fileupload");
...
function fileOnSelect(e) {
    document.querySelector('.file-upload-wrapper').setAttribute('data-text', e.currentTarget.value.replace(/.*(\/|\\)/, ''));
}
...
function getAllImages() {
    ...
    s3.listObjects(params, function (err, data) {
        let url = `https://${data.Name}.s3.amazonaws.com/`;
        let content = data.Contents.sort(function (a, b) {
            return new Date(a.LastModified) - new Date(b.LastModified);
        });
        content.forEach(element => {
            addPost(url, element.Key);
        });
...
function addPost(url, imageid) {
    document.querySelector('.posts').prepend(getPostHTML(url, imageid));
}

function getPostHTML(url, imageid) {
    let htmlPost = `
                ...
                <div class="post__profile">
                    ...
                    <a href="#" target="_blank" class="post__user">Chandan</a>
                </div>
                ...
                <div class="post__medias">
                    <img class="post__media" src="${url + imageid}" alt="Post Content">
                    <div class="moderation-container" id="${imageid}">It is getting proccesed, will be posted shortly</div>
                </div>
                </div>
                ...
                <div class="post__infos">
                    ...
                    <span>Liked by
                        <a class="post__name--underline" href="#">Sourav</a> and
                        <a href="#">73 others</a></span>
                    </div>

                    <div class="post__description">
                    <span>
                        <a class="post__name--underline" href="#" target="_blank">Chandan</a>
                        Made with ❤ for study
                        purposes.
                    </span>
                    </div>
...
function checkForModeration(fileName) {
    setTimeout(function () {
        getAllDataFromDynamoDB(fileName)
    }, 5000)
}



function getAllDataFromDynamoDB(fileName) {
    ...
    dynamoDB.scan({
        ...
        if (error) {
            ...
            result.Items.forEach((item) => {
                let key = item.S3Key.S;
                let div = document.getElementById(key);
                let modernization = item.ModerationLabels.L;
                if (modernization.length > 0) {
                    // Issue with content

                    div.className = 'moderation-container error';
                    div.innerHTML = "This content canot be posted !";
                }
                else {
                    if (div) {
